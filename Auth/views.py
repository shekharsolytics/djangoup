...
from Auth.utility import _wait_for_user_to_spawn, check_jupyterhub_status
...
class StartJupyterhubServer(APIView):
    permission_classes = [IsAuthenticated]

    @staticmethod
    def get(request):
        username = request.user.username
        if check_jupyterhub_status(jupyterhub_url, headers, username):
            return Response(data={"msg": "server already started"}, status=400)
        ...
        if response.status_code in (201, 202):
            ...
            return Response(data={"msg": "server started."}, status=response.status_code)
        return Response(data={"msg": "Something broken."}, status=response.status_code)


class SpawnJupyterhubServer(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(request):
        ...
        try:
            ...
            if server_model:
                ...
                return Response(data={"msg": "Server spawned successfully"}, status=200)
        except Exception as e:
            return Response(data={"msg": "something broken"}, status=500)



class AccessJupyterhubServer(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(request):
        ...
        try:
            # check successfull spawn
            if not check_jupyterhub_status(jupyterhub_url, headers, username):
                return Response(data={"msg": "server not started/spawned."}, status=400)
            
            notebook_url = f"{jupyterhub_url}user/{username}"
            return HttpResponseRedirect(notebook_url)
        except Exception as e:            
            return Response(data={"error": f"Unable to access jupyter server."}, status=500)


class StopJupyterhubServer(APIView):
    permission_classes = [IsAuthenticated]
